async function getData() {
  const DataResponse = await fetch("./보스턴집값.json");
  const Datas = await DataResponse.json();
  let datas = [];
  for (let i = 0; i < Datas.Data.length; i++) {
    let data = {};
    const dataunit = [];
    dataunit.push(Datas.Data[i].CRIM);
    dataunit.push(Datas.Data[i].ZN);
    dataunit.push(Datas.Data[i].INDUS);
    dataunit.push(Datas.Data[i].CHAS);
    dataunit.push(Datas.Data[i].NOX);
    dataunit.push(Datas.Data[i].RM);
    dataunit.push(Datas.Data[i].AGE);
    dataunit.push(Datas.Data[i].DIS);
    dataunit.push(Datas.Data[i].RAD);
    dataunit.push(Datas.Data[i].TAX);
    dataunit.push(Datas.Data[i].PTRATIO);
    dataunit.push(Datas.Data[i].B);
    dataunit.push(Datas.Data[i].LSTAT);
    data.xt = dataunit;
    data.yt = Datas.Data[i].MEDV;
    datas.push(data);
  }
  return datas;
}
